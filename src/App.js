...
export default function App() {
  ...
  function submitGuess(id) {
    ...
    if(validateWord(word)){
      const pigsBulls = countPBs(players.find(p => p.id!=id).word.toLowerCase(),word)
      setPlayers(oldPlayers => oldPlayers.map(player => {
        return player.id == id ?
          {...player,guesses: {...player.guesses,[word]:pigsBulls}, guess: "", }
          : player
      }))
    }
  }
  const words = players.map(p => <div><Word key={p.id} id={p.id} word={p.word} submitWord={submitWord} setWord={setWord} />{canGuess && <Guess id={p.id} guesses={p.guesses} guess={p.guess} setWord={setWord} submitGuess={submitGuess} />}</div>)
  /*React.useEffect(() =>{
